...
class Theme_Register {
	...
	public function tp_register_theme_options() {
		if ( !current_user_can( 'manage_options' ) )  {
			wp_die( __( 'You do not have sufficient permissions to access this page.','architex' ) );
		}
		...
		<form method="post" id="purchase_code_form" class="notice notice-info">
			<div class="theme-register-wrapper">
				<div class="theme-register-header">
					<h2 class="hndle ui-sortable-handle"><?php esc_html_e('Register Theme','architex'); ?></h2>
				</div>
				<p><?php esc_html_e('You\'re almost done. Just one more step. In order to gain full access
					to all demos, premium plugins and support, please register your theme\'s purchase code.','architex'); ?></p>
				<h4><?php esc_html_e('Your Envato Purchase Code','architex'); ?></h4>
				...
				<p>
					<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank"><?php esc_html_e('Where to find the code?','architex'); ?></a>
				</p>
				...
				<p>
					<label>
						...
						installations is a copyright violation.','architex'); ?>
					</label>
					<a href="https://themeforest.net/licenses/terms/regular" target="_blank"><?php esc_html_e('License details.','architex'); ?></a>
				</p>
	
				<p class="tp-actions">
					<button class="button button-primary" name="register_theme"><?php esc_html_e('Register Theme','architex'); ?></button>
				</p>
			</div>
		</form>
		<?php else: ?>
		<div class="wrap">
					...
					<div class="welcome-panel-column-container">
						<div class="welcome-panel-column">
							...
							<div class="welcome-panel-column-content">
								<h3>Theme Support</h3>
								<p><?php echo esc_html__('Envato allows 1 license for 1 project located on 1 domain. It means that 1 purchase key can be used only for 1 site. Each additional site will require an additional purchase key.','architex'); ?>
								</p>
			...
			<form action="" method="post">
				...
				<button type="submit" class="button button-primary" name="deactive_license"><?php esc_html_e('Deactivate License Key', 'architex'); ?></button>
			</form>
		</div>
		<?php endif;
		...
		if(isset($_POST['purchase_code']) && !empty($_POST['purchase_code'])) {
			...
			$url = 'https://api.themepure.net/wp-json/tp-api/v1/check-code?' . http_build_query([
				'code' => $purchase_code,
				'theme' => $theme,
				'version' => $version,
				'author' => $author,
	...
	public function tp_tgmpa_filter_action_links( $action_links, $item_slug, $item, $view_context ) {
		...
		if ( 'External Source' === $source && !self::tp_is_theme_registered() ) {
			...
			$action_links = array(
				'tp_registration_required' => sprintf( __( '<a style="color: red;" href="%s">Register Theme To Unlock It</a>', 'architex' ), esc_url( admin_url( 'themes.php?page=register-theme' ) ) ),
			);
		}
	...
	public function tp_notice_for_activation() {
		if(empty(self::tp_get_registered_purchase_code())){
			echo '<div class="notice notice-warning">
				<p>' . sprintf(
				esc_html__( 'Enter your Envato Purchase Code to receive Theme and plugin updates %s', 'architex' ),
				'<a href="' . admin_url('themes.php?page=register-theme') . '">' . esc_html__( 'Enter Purchase Code', 'architex' ) . '</a>') . '</p>
			</div>';
		}
	}
	...
	public function tp_prevent_modal(){
		...
		<div id="register-modal" class="tp-modal">
			...
			<div class="tp-modal-content">
				...
				<p><?php esc_html_e('Please register theme with your evato purchase code to activate this plugin.', 'architex'); ?> <a href="<?php echo esc_url( admin_url( 'themes.php?page=register-theme' ) ); ?>"><?php esc_html_e('Register Theme', 'architex'); ?></a></p>
			</div>
