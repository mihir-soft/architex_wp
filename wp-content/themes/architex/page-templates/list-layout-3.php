<?php 

/**
 * Template Name: Portfolio - List Layout 3
 * @package architex 
 */

get_header();

?>

<div class="portfolio-section section-padding">
    <div class="container">
        <div class="row">
            <div class="col-xl-4 col-lg-4 col-md-6">
                <div class="single-portfolio-wrapper">
                    <div class="portfolio-img">
                        <img src="<?php echo esc_url( get_template_directory_uri() );?>/assets/img/portfolio/1.jpg" alt="">
                    </div>
                    <div class="portfolio-title">
                        <h3>Artistic</h3>
                        <h6>Business</h6>
                    </div>
                </div>
            </div>
                    
            <div class="col-xl-4 col-lg-4 col-md-6">
                <div class="single-portfolio-wrapper">
                    <div class="portfolio-img">
                        <img src="<?php echo esc_url( get_template_directory_uri() );?>/assets/img/portfolio/2.jpg" alt="">
                    </div>
                    <div class="portfolio-title">
                        <h3>Nemesis</h3>
                        <h6>Business</h6>
                    </div>
                </div>
            </div>
                    
            <div class="col-xl-4 col-lg-4 col-md-6">
                <div class="single-portfolio-wrapper">
                    <div class="portfolio-img">
                        <img src="<?php echo esc_url( get_template_directory_uri() );?>/assets/img/portfolio/3.jpg" alt="">
                    </div>
                    <div class="portfolio-title">
                        <h3>Warfaze</h3>
                        <h6>Business</h6>
                    </div>
                </div>
            </div>
                    
            <div class="col-xl-4 col-lg-4 col-md-6">
                <div class="single-portfolio-wrapper">
                    <div class="portfolio-img">
                        <img src="<?php echo esc_url( get_template_directory_uri() );?>/assets/img/portfolio/4.jpg" alt="">
                    </div>
                    <div class="portfolio-title">
                        <h3>Renessa</h3>
                        <h6>Business</h6>
                    </div>
                </div>
            </div>

            <div class="col-xl-4 col-lg-4 col-md-6">
                <div class="single-portfolio-wrapper">
                    <div class="portfolio-img">
                        <img src="<?php echo esc_url( get_template_directory_uri() );?>/assets/img/portfolio/5.jpg" alt="">
                    </div>
                    <div class="portfolio-title">
                        <h3>Renessa</h3>
                        <h6>Business</h6>
                    </div>
                </div>
            </div>

            <div class="col-xl-4 col-lg-4 col-md-6">
                <div class="single-portfolio-wrapper">
                    <div class="portfolio-img">
                        <img src="<?php echo esc_url( get_template_directory_uri() );?>/assets/img/portfolio/6.jpg" alt="">
                    </div>
                    <div class="portfolio-title">
                        <h3>Renessa</h3>
                        <h6>Business</h6>
                    </div>
                </div>
            </div>

            <div class="col-xl-4 col-lg-4 col-md-6">
                <div class="single-portfolio-wrapper">
                    <div class="portfolio-img">
                        <img src="<?php echo esc_url( get_template_directory_uri() );?>/assets/img/portfolio/7.jpg" alt="">
                    </div>
                    <div class="portfolio-title">
                        <h3>Artcell</h3>
                        <h6>Business</h6>
                    </div>
                </div>
            </div>

            <div class="col-xl-4 col-lg-4 col-md-6">
                <div class="single-portfolio-wrapper">
                    <div class="portfolio-img">
                        <img src="<?php echo esc_url( get_template_directory_uri() );?>/assets/img/portfolio/8.jpg" alt="">
                    </div>
                    <div class="portfolio-title">
                        <h3>Pentagun</h3>
                        <h6>Business</h6>
                    </div>
                </div>
            </div>

            <div class="col-xl-4 col-lg-4 col-md-6">
                <div class="single-portfolio-wrapper">
                    <div class="portfolio-img">
                        <img src="<?php echo esc_url( get_template_directory_uri() );?>/assets/img/portfolio/9.jpg" alt="">
                    </div>
                    <div class="portfolio-title">
                        <h3>Siriniya</h3>
                        <h6>Business</h6>
                    </div>
                </div>
            </div>
            
        </div>
    </div>
</div>

<?php 

get_footer();

